...
object AtomicDef {
  ...
  val columns = List(
    ...
    Column("event_id", Char(Some(36)), notNull = true, unique = true),
    ...
    // Location
    Column("geo_country", Char(Some(2))),
    Column("geo_region", Char(Some(2))),
    ...
    // Session ID
    Column("domain_sessionid", Char(Some(128))),
